...
const Piebranchorders = ({ todayroutes }) => {
  ...
  // Process data for orders (stops-based calculation)
  const chartData = useMemo(() => {
    ...
    todayroutes.forEach((route) => {
      const branchName = route.branch?.name || "Unknown Branch";
      ...
      route.stops?.forEach((stop) => {
        branchStats[branchKey].total += 1;

        if (stop.iscompleted && !stop.iscancelled) {
          branchStats[branchKey].completed += 1;
        } else if (stop.iscancelled) {
          branchStats[branchKey].cancelled += 1;
        } else {
          branchStats[branchKey].pending += 1;
        }
      });
    });
      ...
      >
        ...
        <div style={{
          ...
          <div style={{
            ...
            >
              {auth.role === 'superadmin' ? "Branch Orders Distribution" : "Order Statistics"}
            </div>
          </div>
          ...
          <div style={{
            ...
            Orders breakdown by status and branch
          </div>
        ...
        {chartData.length === 0 && (
          <div style={{
            ...
            <div style={{
              ...
              Order statistics will appear here once stops are completed
            </div>
          </div>
        )}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    isAnimationActive={isInView && !hasAnimated}
                  >
...
export default Piebranchorders;